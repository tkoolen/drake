classdef AtlasAnkleXYJointLimitConstraint < SingleTimeLinearPostureConstraint
  ...
  methods
    function obj = AtlasAnkleXYJointLimitConstraint(robot, side)
      if ~strcmp(side, 'l') && ~strcmp(side, 'r')
        error('side not recognized');
      end

      leg_akx = robot.findPositionIndices([side '_leg_akx']);
      leg_aky = robot.findPositionIndices([side '_leg_aky']);
      ...
      iAfun = [(1:num_lin_ineq)';(1:num_lin_ineq)'];
      jAvar = reshape(bsxfun(@times,ones(num_lin_ineq,1),[leg_akx leg_aky]),[],1);
      Aval = reshape(ankle_limits_A,[],1);
      obj = obj@SingleTimeLinearPostureConstraint(robot,iAfun,jAvar,Aval,-inf(num_lin_ineq,1),ankle_limits_b);
