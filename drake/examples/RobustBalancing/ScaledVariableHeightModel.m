classdef ScaledVariableHeightModel < NStepCapturabilitySOSSystem
  ...
  methods
    function obj = ScaledVariableHeightModel(g, z_nom, step_max, step_time, f_range)
    ...
    % f: 'leg force'
    % m * vdot = [0; -mg] + f
    % define u = f_range^-1*(f_z / (m * g * z) - 1/z_nom)
    % vdot = [0; -g] + f_range * (u + 1/z_nom) * q * g
    % using x(2) = z - z_nom
    function xdot = dynamics(obj, t, x, u)
      ...
      vdot = [q(1)/obj.z_nom*(obj.f_range*u*obj.g + obj.g); obj.f_range*u*obj.g];
      xdot = [v; vdot];
    end
