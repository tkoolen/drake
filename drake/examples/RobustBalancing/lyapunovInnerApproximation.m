function [Vsol,qsol] = lyapunovInnerApproximation(model,W,T,u,R_diag,options)
...
% (1) w(x) >= 0, x in X ==> Vdot <= 0
[prog, vdot_sos] = spotless_add_sprocedure(prog, -Vdot-beta*V, h_X,V_vars,Vdot_degree-2);
[prog, vdot_sos] = spotless_add_sprocedure(prog, vdot_sos, W,V_vars,Vdot_degree-W_degree);
...
% [prog, vmax_sos] = spotless_add_sprocedure(prog, vmax_sos, W,V_vars,degree-W_degree);
...
[prog, vmax_sos] = spotless_add_eq_sprocedure(prog, vmax_sos, W,V_vars,degree-W_degree);
[prog, vmax_sos] = spotless_add_sprocedure(prog, vmax_sos, t*(1-t),V_vars,degree-2);
prog = prog.withSOS(vmax_sos);

% (2b) w(x) >= 0, x in dX ==> V >= 1
...
% [prog, v_sos] = spotless_add_sprocedure(prog, v_sos, W,V_vars,degree-W_degree);
