...
[PLOT1,PLOT2] = meshgrid(linspace(-R_diag(1),R_diag(1),400),linspace(-R_diag(3),R_diag(3),400));
% h_goal_val = reshape(msubs(subs(V0p,sub_vars,sub_val),x([1 3]),[Q(:)';QD(:)']),size(Q,1),[]);
h_X_val = reshape(msubs(subs(h_X,sub_vars,sub_val),plot_vars,[PLOT1(:)';PLOT2(:)']),size(PLOT1,1),[]);
% h_Xp_val = reshape(msubs(subs(subs(h_X,x,xp),sub_vars,sub_val),x([1;3]),[Q(:)';QD(:)']),size(Q,1),[]);

Wval = reshape(msubs(subs(Wsol,sub_vars,sub_val),plot_vars,[PLOT1(:)';PLOT2(:)']),size(PLOT1,1),[]);
figure(1)
hold off
[cl,h]=contour(PLOT1,PLOT2,Wval,[1 1],'b');
clabel(cl,h);
hold on
% [cl,h]=contour(Q,QD,h_goal_val,[0 0],'k');
[cl,h]=contour(PLOT1,PLOT2,h_X_val,[0 0],'r');
% [cl,h]=contour(Q,QD,h_Xp_val,[0 0],'r');
hold off

Vval = reshape(msubs(subs(Vsol,sub_vars,sub_val),plot_vars,[PLOT1(:)';PLOT2(:)']),size(PLOT1,1),[]);
% Vval2 = reshape(msubs(subs(subs(Vsol,x,xp),sub_vars,sub_val),x([1;3]),[Q(:)';QD(:)']),size(Q,1),[]);
figure(22)
hold off
[cl,h]=contour(PLOT1,PLOT2,Vval,[0 0],'b');
clabel(cl,h);
hold on
...
inds = [2;1];
assert(double(sum(Vpvars(inds) - [q(2);v(1)])) == 0)
Vppows = [zeros(size(Vppows,1),1) Vppows(:,inds) zeros(size(Vppows,1),1)];

[dVpdxvars,dVpdxpows,dVpdxcoeffs] = decomp(diff(V0p,x));
inds = [2;1];
assert(double(sum(dVpdxvars(inds) - [q(2);v(1)])) == 0)
dVpdxpows = [zeros(size(dVpdxpows,1),1) dVpdxpows(:,inds) zeros(size(dVpdxpows,1),1)];

save controller_V1 dVducoeffs dVdupows Vpcoeffs Vppows dVpdxcoeffs dVpdxpows
