...
void RobotStateDecoder::EvalOutput(const Context<double> &context,
  ...
  const auto& message =
      EvalAbstractInput(context, robot_state_message_port_.get_index())
          ->GetValue<robot_state_t>();

  // Output: KinematicsCache.
  auto& kinematics_cache = output->GetMutableData(kinematics_cache_port_.get_index())
      ->GetMutableValue<KinematicsCache<double>>();

  VectorXd q(tree_.get_num_positions()); // TODO(tkoolen): don't allocate anew.
  VectorXd v(tree_.get_num_velocities()); // TODO(tkoolen): don't allocate anew.

  // Floating joint.
  if (floating_body_) {
    auto floating_body_to_world = DecodePose(message.pose);
    auto floating_body_twist_in_in_world_aligned_body = DecodeTwist(message.twist);
    ...
    if (floating_joint.get_num_positions() == kRpySize + kSpaceDimension) {
      ...
      // Translation.
      q.segment<kSpaceDimension>(position_start) = floating_body_to_world.translation();
      ...
      auto translationdot = floating_body_twist_in_in_world_aligned_body.tail<kSpaceDimension>();
      ...
      auto angular_velocity_world = floating_body_twist_in_in_world_aligned_body.head<kSpaceDimension>();
      auto rpydot = (phi * angular_velocity_world).eval();
      v.segment<kRpySize>(velocity_start + kSpaceDimension) = rpydot;
    }
    else if (floating_joint.get_num_positions() == kQuaternionSize + kSpaceDimension) {
      ...
      world_aligned_body_to_body.linear() = floating_body_to_world.linear();
      ...
      TwistVector<double> floating_body_twist_in_body =
          transformSpatialMotion(world_aligned_body_to_body, floating_body_twist_in_in_world_aligned_body);
...
RobotStateDecoder::AllocateOutput(const Context<double> &context) const {
